...
public class GAEProxyActivity extends PreferenceActivity implements
	...
	final Handler handler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case MSG_CRASH_RECOVER:
				Crouton.makeText(GAEProxyActivity.this, R.string.crash_alert,
						Style.ALERT).show();
	...
	private void copyAssets(String path) {
		...
		for (int i = 0; i < files.length; i++) {
			...
			try {
				in = assetManager.open(path + (path.isEmpty() ? "" : "/")
						+ files[i]);
	...
	private boolean install() {
		...
		copyAssets("modules");
		String[] argc = { "7z", "x", tmp.getAbsolutePath(),
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		// Create the adView
		adView = new AdView(GAEProxyActivity.this, AdSize.SMART_BANNER,
				"a14d8be8a284afc");
	...
	private void showAbout() {
		...
		new AlertDialog.Builder(this)
				.setTitle(
						String.format(getString(R.string.about_title),
								versionName))
				.setCancelable(false)
				.setNegativeButton(getString(R.string.ok_iknow),
						new DialogInterface.OnClickListener() {
